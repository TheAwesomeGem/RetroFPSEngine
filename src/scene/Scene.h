...
namespace Scene
{
    ...
    struct SceneState
    {
        std::vector<Actor> actors = {}; // TODO: Use a tree or something. This will be slow and heavy.
        std::vector<size_t> actor_free_indices = {};
        ActorHandle camera_handle = ActorHandle::invalid();
    };

    void load(SceneState& scene);
    void update(SceneState& scene, double delta_time);
    void render(const SceneState& scene, const Renderer::RendererState& renderer);
    ...
    void destroy_actor(SceneState& scene, ActorHandle handle, bool should_destroy_childrens);
    [[nodiscard]] Mat4 get_local_to_world_mat(const SceneState& scene, const Actor* actor);
    [[nodiscard]] inline bool is_actor_exists(const SceneState& scene, ActorHandle handle)
     {
         if (!handle.is_valid())
         {
             return false;
         ...
         if (handle.index >= std::size(scene.actors))
         {
             return false;
         }

         const Actor& actor = scene.actors[handle.index];
    ...
    [[nodiscard]] inline const Actor* get_actor(const SceneState& scene, ActorHandle handle)
     {
         if (!is_actor_exists(scene, handle))
         {
             return nullptr;
         }

         return &scene.actors[handle.index];
     }
    [[nodiscard]] inline Actor* get_actor(SceneState& scene, ActorHandle handle)
    {
        if (!is_actor_exists(scene, handle))
        {
            return nullptr;
    ...
    inline void attach_mesh_render(SceneState& scene, ActorHandle handle, const StaticMeshRender& mesh_render)
    {
    ...
    inline void attach_camera(SceneState& scene, ActorHandle handle, const Camera& camera)
    {
        Actor* actor = get_actor(scene, handle);
        assert(actor != nullptr);
